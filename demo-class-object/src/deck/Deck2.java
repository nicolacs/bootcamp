...
public class Deck2 {
  ...
  public Deck2(Game game) {
    // Parent: Rank, Child: BigTwoRank
    // Rank[] and BigTwoRank[] ?

    // Shape[] vs Circle[]
    NewRank[] ranks = new NewRank[0];
    if (game == Game.BIGTWO) {
    ...
    for (Suit2 suit : Suit2.values()) { // 4 times
      for (NewRank rank : ranks) { // 13 times
  ...
  public static void main(String[] args) {
    Deck2 deck = new Deck2(Game.POKER); // Card ?
    ...
    NewRank[] ranks1 = new BigTwoRank[] {BigTwoRank.ofACE(), BigTwoRank.ofTWO()};
    System.out.println(ranks1[0].compareTo(ranks1[1])); // -1

    NewRank[] ranks2 = new PokerRank[] {PokerRank.ofACE(), PokerRank.ofTWO()};
    

    NewRank[] bigTwoRanks = new NewRank[] {BigTwoRank.ofACE(), BigTwoRank.ofTWO()}; // OK
